...
class KioskApp(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("{}x{}".format(width, height)) # 화면 크기 설정

        # 첫 번째 화면 생성
        self.start_frame = ctk.CTkFrame(self)
        self.start_frame.pack(expand=True, fill="both", padx=10, pady=10)

        self.touch_button = ctk.CTkButton(self.start_frame, text="click me!", font=("Arial", buttonSize0), command=self.show_menu_screen)
        ...
        for category in self.categories:
            ctk.CTkButton(self.category_frame, text=category, font=("Arial", buttonSize1)).pack(side="left", padx=5, pady=5, expand=True)
        ...
        for item, price in self.menu_items:
            ctk.CTkButton(self.menu_frame, text=f"{item} - {price}", font=("Arial", buttonSize2)).pack(fill="x", padx=5, pady=5)

        # 하단 주문 정보와 결제 버튼
        self.bottom_frame = ctk.CTkFrame(self.main_frame)
        self.bottom_frame.pack(side="bottom", fill="x")

        self.order_info = ctk.CTkLabel(self.bottom_frame, text="주문상품: 0개 | 주문금액: 0원", font=("Arial", buttonSize1))
        self.order_info.pack(side="left", padx=10)

        self.payment_button = ctk.CTkButton(self.bottom_frame, text="결제하기", font=("Arial", buttonSize2), fg_color="red")
